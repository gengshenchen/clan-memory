...
export const useMedia = (selectedMember: FamilyMember | null) => {
  ...
  useEffect(() => { memberRef.current = selectedMember; }, [selectedMember]);
  useEffect(() => { typeRef.current = mediaType; }, [mediaType]);
  ...
  useEffect(() => {
    ...
    window.onResourceImported = (data) => {
      ...
      if (currentMember && currentType) {
        window.CallBridge?.invoke("fetchMemberResources", currentMember.id, currentType);
      }
    };
  }, []);
